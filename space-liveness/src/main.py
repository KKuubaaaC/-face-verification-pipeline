...
# ─── Eager init pipeline przy starcie (modele ładowane PRZED uruchomieniem UI) ─
logger.info("Ładowanie modeli insightface — proszę czekać...")
_pipeline = VerificationPipeline()
logger.info("Modele gotowe. Uruchamianie UI...")
...
def analyze_two_images(
    ...
    try:
        ...
        logging.getLogger(__name__).error("Błąd analyze_two_images: %s", exc, exc_info=True)
...
def build_ui() -> gr.Blocks:
    with gr.Blocks(title="Analiza Badawcza — Embeddingi") as demo, gr.Tabs():
        with gr.Tab("🔬 Analiza Badawcza — Embeddingi"):
            gr.Markdown(
                ...
                f"i zestawimy go z progiem EER {VERIFICATION_THRESHOLD} (ArcFace)."
            )
